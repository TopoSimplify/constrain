...
func TestByMinDistRelation(t *testing.T) {
	...
	g.Describe("constrain by mindist relation", func() {
		g.It("should test constrain by context geometry", func() {
			...
			g.Assert(ByMinDistRelation(options, hull, cg_a)).IsTrue() // expands mindist
			g.Assert(ByMinDistRelation(options, hull, cg_b)).IsFalse() //reduces mindist
			g.Assert(ByMinDistRelation(options, hull, cg_c)).IsTrue() //expands mindist
		})
	})
}
